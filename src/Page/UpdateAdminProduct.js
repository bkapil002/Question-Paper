...
const UpdateAdminProduct = ({onClose,
  deltails,
  fetchdata}) => {
  const [data, setData] = useState({
    ...deltails,
    subject: deltails?.subject,
    year: deltails?.year,
    semester: deltails?.semester,
    paper: deltails?.paper,
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const dataResponse = await fetch(UserLink.updatePaper.url, {
        method: UserLink.updatePaper.method,
        credentials: "include",
        headers: {
